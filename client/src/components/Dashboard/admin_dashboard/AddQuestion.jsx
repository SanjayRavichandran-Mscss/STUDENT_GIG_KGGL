  ...
  const handleAddQuestion = async () => {
    ...
    try {
      const questionToSave = {
        skill_id: parseInt(formData.skillId),
        difficulty_level_id: parseInt(formData.difficultyLevelId),
        questions: `<p>${formData.questionText}</p>`, // Backend expects "questions"
        option: formData.options, // Backend expects "option" as array of {option, feedback}
        correct_answer: formData.correctAnswer,
      };
      ...
      console.error("Error saving question:", error);
      const errorMessage =
        error.response?.data?.msg || "Failed to save question to database";
