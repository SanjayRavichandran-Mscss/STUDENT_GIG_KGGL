...
const ReceivableLedger = () => {
  ...
  const validateForm = () => {
    const { projectId, projectName, paidAmount, fromUpiId, toUpiId, transactionId, dateTime, transactionScreenshot } = formData;

    if (!projectId || !projectName || !paidAmount || !fromUpiId || !toUpiId || !transactionId || !dateTime || !transactionScreenshot) {
      MySwal.fire({
        icon: 'error',
        title: 'Missing Fields',
        text: 'Please fill all required fields.',
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {projectTransactions.map(transaction => (
                            <tr key={transaction.id} className="hover:bg-gray-50 transition duration-150">
                              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                {transaction.client_name} {/* This now comes from projects table via JOIN in backend */}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-medium">
                                ₹{parseFloat(transaction.paid_amount).toFixed(2)}
                              </td>
