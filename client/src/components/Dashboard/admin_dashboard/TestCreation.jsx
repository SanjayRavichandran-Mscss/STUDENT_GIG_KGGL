...
//           fetch("https://gig.kggeniuslabs.com/apiapi/test/skills", { credentials: 'include' }),
//           fetch("https://gig.kggeniuslabs.com/apiapi/test/difficulty-levels", { credentials: 'include' }),
//           fetch("https://gig.kggeniuslabs.com/apiapi/test/available-questions", { credentials: 'include' }),
//         ]);
...
//         distribution.medium_level_question = totalQuestions - (manualPassCriteria ? Number(formData.easy_pass_mark) : passMarks.easy_pass_mark);
...
//         const easyPass = manualPassCriteria ? Number(formData.easy_pass_mark) : passMarks.easy_pass_mark;
...
//         const mediumPass = manualPassCriteria ? Number(formData.medium_pass_mark) : passMarks.medium_pass_mark;
...
//     }
//   }, [formData.difficulty_level_id, formData.total_no_of_questions, formData.easy_pass_mark, formData.medium_pass_mark, manualPassCriteria]);
...
//       if (Number(formData.easy_pass_mark) > questionDistribution.easy_level_question) {
...
//       }
//       if (Number(formData.medium_pass_mark) > questionDistribution.medium_level_question) {
...
//       }
//       if (Number(formData.hard_pass_mark) > questionDistribution.hard_level_question) {
...
//         const remainingAfterEasy = formData.total_no_of_questions - Number(formData.easy_pass_mark);
...
//         const remainingAfterMedium = formData.total_no_of_questions - Number(formData.medium_pass_mark) - Number(formData.easy_pass_mark);
...
//   }, [formData.skill_id, formData.total_no_of_questions, formData.easy_pass_mark, formData.medium_pass_mark, formData.hard_pass_mark, formData.difficulty_level_id, questionDistribution]);
...
//       const response = await fetch("https://gig.kggeniuslabs.com/apiapi/test/create-test", {
//         method: 'POST',
...
export default function TestCreation() {
  ...
  // Fetch skills and available questions
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
  ...
  // Validate form and question availability
  useEffect(() => {
    const errors = {};
    const duration = Number(formData.duration_minutes);
    if (duration < 0) errors.duration_minutes = "Test duration cannot be negative.";
    ...
    if (formData.skill_id) {
      ...
      if (selectedSkillQuestions.easy_count < requiredQuestions.easy) {
        errors.questions = `Not enough Easy questions. Need ${requiredQuestions.easy}, but only ${selectedSkillQuestions.easy_count} available.`;
      } else if (selectedSkillQuestions.medium_count < requiredQuestions.medium) {
        errors.questions = `Not enough Medium questions. Need ${requiredQuestions.medium}, but only ${selectedSkillQuestions.medium_count} available.`;
      } else if (selectedSkillQuestions.hard_count < requiredQuestions.hard) {
        errors.questions = `Not enough Hard questions. Need ${requiredQuestions.hard}, but only ${selectedSkillQuestions.hard_count} available.`;
      }
    }

    setValidationErrors(errors);
  }, [formData.skill_id, formData.duration_minutes, selectedSkillQuestions]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dataToSubmit = {
        ...
        skill_id: Number(formData.skill_id),
        duration_minutes: Number(formData.duration_minutes),
        difficulty_level_id: 3, // Hard level
                ...
                <div>
                  ...
                  {formData.skill_id && (
                    <div className="text-xs text-gray-600 mt-2">
                      ...
                      <p>
                        <span className="font-medium text-blue-600">Easy:</span> {selectedSkillQuestions.easy_count} (Required: 20)
                      </p>
                      <p>
                        <span className="font-medium text-green-600">Medium:</span> {selectedSkillQuestions.medium_count} (Required: 10)
                      </p>
                      <p>
                        <span className="font-medium text-red-600">Hard:</span> {selectedSkillQuestions.hard_count} (Required: 4)
                      </p>
                    </div>
                  )}
                </div>
