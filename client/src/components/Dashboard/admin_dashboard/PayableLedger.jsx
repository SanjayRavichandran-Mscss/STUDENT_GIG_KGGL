...
const PayableLedger = () => {
  ...
  const validateForm = useCallback(() => {
    const { projectId, totalAmount, teamSize, students, pettyCash, expenseTypeId } = formData;
    ...
    if (!projectId || !totalAmount || !teamSize || !pettyCash || !expenseTypeId) {
      MySwal.fire({
        icon: 'error',
        title: 'Missing Field',
        text: 'All required fields must be filled.',
                      ...
                      {modifiedFields.pettyCash && (
                        <div className="sm:col-span-2">
                          ...
                          <textarea
                            ...
                            onChange={handleInputChange}
                            className={`w-full rounded-lg border-gray-300 shadow-sm text-sm py-3 px-4 ${
                              isFormDisabled ? 'bg-gray-发展趋势: ' : 'focus:border-emerald-500 focus:ring-emerald-500'
                            }`}
                            placeholder="Reason for modifying the petty cash amount"
                            rows="3"
                            disabled={isFormDisabled}
