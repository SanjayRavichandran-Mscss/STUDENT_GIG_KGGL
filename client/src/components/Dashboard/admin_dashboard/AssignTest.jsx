...
export default function AssignTest() {
    ...
    const fetchData = async () => {
      ...
      try {
        const [testsResponse, studentsResponse, questionsResponse] = await Promise.all([
          ...
          axios.get("http://localhost:5000/api/test/students", { withCredentials: true }),
        ...
        const assignedPromises = testsResponse.data.map((test) =>
          axios.get(`http://localhost:5000/api/test/assigned-students/${test.test_id}`, {
  ...
  const handleAssignTest = async (testId) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/test/assign-test",
  ...
  const handleToggleStatus = async (testId, currentStatus) => {
    try {
      ...
      await axios.post(
        "http://localhost:5000/api/test/toggle-test-status",
