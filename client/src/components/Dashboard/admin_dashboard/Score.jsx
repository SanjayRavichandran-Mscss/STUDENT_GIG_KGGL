...
export default function Score() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        // Include decodedId in API call if backend supports it
        // Example: const response = await axios.get(`http://localhost:5000/api/stu/all-students-test-data/${decodedId}`);
        const response = await axios.get('http://localhost:5000/api/stu/all-students-test-data', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('accessToken')}`, // Include token
            'X-Admin-ID': decodedId, // Pass admin ID as header for validation
          },
        });
        if (response.data?.status === 'success' && Array.isArray(response.data.students)) {
          setData(response.data);
