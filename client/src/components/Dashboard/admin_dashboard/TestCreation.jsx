...
//       errors.easy = `Only ${selectedSkillQuestions.easy_count} Easy questions available. Need ${formData.easy_level_question - selectedSkillQuestions.easy_count} more.`;
//     }
//     if (Number(formData.medium_level_question) > selectedSkillQuestions.medium_count) {
//       errors.medium = `Only ${selectedSkillQuestions.medium_count} Medium questions available. Need ${formData.medium_level_question - selectedSkillQuestions.medium_count} more.`;
//     }
//     if (Number(formData.hard_level_question) > selectedSkillQuestions.hard_count) {
//       errors.hard = `Only ${selectedSkillQuestions.hard_count} Hard questions available. Need ${formData.hard_level_question - selectedSkillQuestions.hard_count} more.`;
//     }
...
export default function TestCreation() {
  ...
  useEffect(() => {
    const errors = {};
    if (formData.skill_id) {
      const easyShortage = Number(formData.easy_level_question) - selectedSkillQuestions.easy_count;
      const mediumShortage = Number(formData.medium_level_question) - selectedSkillQuestions.medium_count;
      const hardShortage = Number(formData.hard_level_question) - selectedSkillQuestions.hard_count;

      if (easyShortage > 0) {
        errors.easy = `Please add ${easyShortage} more Easy questions. Only ${selectedSkillQuestions.easy_count} available.`;
      }
      if (mediumShortage > 0) {
        errors.medium = `Please add ${mediumShortage} more Medium questions. Only ${selectedSkillQuestions.medium_count} available.`;
      }
      if (hardShortage > 0) {
        errors.hard = `Please add ${hardShortage} more Hard questions. Only ${selectedSkillQuestions.hard_count} available.`;
      }
      if (Number(formData.easy_pass_mark) > Number(formData.easy_level_question)) {
        errors.easy_pass = `Easy pass mark cannot exceed ${formData.easy_level_question}`;
      }
