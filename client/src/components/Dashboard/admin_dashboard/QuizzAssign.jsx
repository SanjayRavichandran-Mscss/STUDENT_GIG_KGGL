...
function QuizzAssign() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get("http://103.118.158.24/api/api/admin/categories");
      ...
      if (formData.selectedCategoryId) {
        try {
          const response = await axios.get(
            `http://103.118.158.24/api/admin/api/questions/count?category_id=${formData.selectedCategoryId}`
          );
  ...
  const handleAssignQuiz = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://103.118.158.24/api/api/admin/assign-test", data);
