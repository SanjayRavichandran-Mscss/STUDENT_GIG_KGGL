...
export default function MyTest() {
    ...
    const fetchData = async () => {
      try {
        ...
        const testsResponse = await axios.get(
          `http://103.118.158.24/api/api/quiz/all-tests/${studentId}`,
        ...
        const schedulesResponse = await axios.get(
          `http://103.118.158.24/api/api/quiz/schedules/${studentId}`,
          { withCredentials: true }
        );
        ...
        const attendedResponse = await axios.get(
          `http://103.118.158.24/api/api/quiz/student-test-attended/${studentId}`,
          { withCredentials: true }
        );

        setAttendedTests(attendedResponse.data.attended_tests);
      } catch (err) {
        setError(err.response?.data?.msg || "Failed to fetch data. Please try again later.");
  ...
  const handleDateTimeChange = async (testId, date) => {
    ...
    try {
      ...
      await axios.post(
        "http://103.118.158.24/api/api/quiz/schedule",
        {
          ...
          {tests.map((test) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  {isAttended && schedules[test.test_id] ? (
                    <p className="text-sm text-gray-600">
                      {schedules[test.test_id].format("DD-MM-YYYY HH:mm")}
                    </p>
                  ) : (
                    !isAttended && (
                ...
                <button
                  onClick={() => !isAttended && handleAttendTest(test.test_id, test.type)}
                  disabled={isAttended || !enabledTests[test.test_id] || !!test.error}
                  className={`w-full mt-4 py-3 px-4 rounded-lg font-medium text-white transition duration-200 ${
                    isAttended || !enabledTests[test.test_id] || test.error
