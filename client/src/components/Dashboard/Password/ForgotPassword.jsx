...
export default function ForgotPassword() {
  ...
  const checkEmailExists = async (email) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/stu/check-email", { email });
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post("http://localhost:5000/stu/forgot", { Email: email });
  ...
  const handlePasskeySubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post("http://localhost:5000/stu/verify-passkey", { email, passkey });
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post("http://localhost:5000/stu/reset", {
        email,
        passkey,
